...
try {
  ...
  const variant = races.find((r) => r.status === 'fulfilled').value;
  ...
  const offers = new OffersService(
    ...
    sRepo,
    db.getRepository(ProductVariant),
  );
  const created = [];
  for (let i = 0; i < 3; i++)
    created.push(
  ...
  assert.equal(
    (await offers.findAll({ variantId: variant.id })).items.length,
    3,
  );
  assert.equal(
  ...
  await assert.rejects(
    variants.update(variant.id, {
      attributeValueIds: [values[2].id, values[1].id],
    }),
    (e) => e.getStatus() === 409,
  );
  ...
  for (const field of ['sku', 'price', 'minPrice', 'maxPrice', 'stockQuantity'])
    assert.ok(
      !doc.components.schemas.CreateProductAttributeDto.properties[field],
      field,
    );
  ...
  await app.close();
  console.log(
    'PASS: migration up/down/up with archives, concurrent combinations, three sellers per variant, stock validation, tenant isolation, price Excel round-trip, real order snapshots and shipping, dependency injection and Swagger.',
  );
} finally {
