...
import type { ListWithdrawsQueryDto } from './dto/withdraw.dto.js';
...
export class DepositsService {
  ...
  async listWithdrawsPaged(query: ListWithdrawsQueryDto, userId?: string) {
    const { page, limit, offset } = getPaginationParams(query);
  ...
  async reviewWithdraw(withdrawId: string, action: 'approve' | 'reject') {
    const result = await this.dataSource.transaction(async (manager) => {
      const withdrawRepo = manager.getRepository(Withdraw);
